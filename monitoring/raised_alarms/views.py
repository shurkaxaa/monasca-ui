...
from monitoring.raised_alarms import forms as alarm_forms
from monitoring.raised_alarms import tables as alarm_tables
...
def generate_status(request):
    try:
        alarms = api.monitor.raised_alarm_list(request)
...
class AlarmServiceView(tables.DataTableView):
    ...
    def get_data(self):
        ...
        if self.service == default_service:
            try:
                results = api.monitor.raised_alarm_list(self.request, page_offset,
                                                 limit)
            ...
            if self.service[:2] == 'id':
                ...
                try:
                    ...
                    results = api.monitor.raised_alarm_list_by_dimension(self.request,
                                                                  self.service,
                                                                  page_offset,
                                                                  limit)
                except Exception:
